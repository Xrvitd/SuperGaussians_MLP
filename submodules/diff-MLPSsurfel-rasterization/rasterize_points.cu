...
//   torch::Tensor MLPopa = torch::full({P}, 0.0, float_opts);
...
//   const int Nsh = sh.size(0);
  ...
  torch::Tensor dL_dopacity = torch::zeros({P, 4}, means3D.options());
  torch::Tensor dL_dtransMat = torch::zeros({P, 9}, means3D.options());
  torch::Tensor dL_dsh = torch::zeros({P, M + 8, 3}, means3D.options());
