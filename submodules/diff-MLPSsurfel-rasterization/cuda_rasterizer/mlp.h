...
#include <cublas_v2.h>
using namespace std;
#define checkCUDAErrorWithLine(msg) checkCUDAErrorFn(msg, FILENAME, __LINE__)
//namespace Timer {
...
class Net {
	...
	public:
		...
		void forward(Net* net, double *data, int n); // returns class
		void backprop(double *y); 
		double loss(double *y_pred, double *y);
		...
		void gpu_blas_mmul(cublasHandle_t handle, const double *A, const double *B, double *C, const int m, const int k, const int n, bool trans_flag_a = false, bool trans_flag_b = false);
		// reduction invocation code
		void reduction(Net* net,int n, double *dev_odata);
	};